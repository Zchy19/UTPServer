define(['durandal/app', 'knockout', 'jquery', 'komapping','services/utpService', 'services/selectionManager', 
    'services/reportService','services/notificationService', 'jsoneditor', 'lodash','datepicker'],
            function(app, ko, $, komapping, utpService,selectionManager, reportService, notificationService, JSONEditor, _, datepicker) {
                ...
                function TestsetElectricReportViewModel() {
                    var self = this;				
                    ...
                    this.testsetElectricAutoFlag=ko.observable(false);
                      ...
                      if (data != null && data.status === 1) {
                        ...
                        if (self.selectedTestset() == undefined) {
                          ...
                          if(self.aotoFirst){
                            ...
                            self.createTestsetElectricStatistics();
                    ...
                    this.startAutoRefresh = function() {
                        if (self.testsetElectricAutoFlag()) {
                    ...
                    this.testsetElectricAutoFlag.subscribe(function(newValue) {
                    ...
                    this.protocolConditionSetting = function(){
                      self.getTestSetByProject();
                        $('#testsetElectricReportSettingModal').modal('show');
                    };
                    this.showChart=ko.observable(false);
                    this.myChart = null;
                    this.chartOptions = {};
                    this.testsetElectricSeriesData = ko.observableArray([]);
                    this.testsetElectricOption = ko.computed(function() {
                      var seriesData = self.testsetElectricSeriesData();
                      var totalTime = 0;
                    ...
                    this.initChart = function() {
                       var width = document.getElementById('testsetElectricWidth').offsetWidth-20;
                        //设置id为testsetLine的div元素的宽高
                        document.getElementById('testsetElectric').style.width = width + 'px';
                        self.myChart = echarts.init(document.getElementById('testsetElectric'),'walden');
                        self.myChart.setOption(self.testsetElectricOption());
                        // 订阅计算属性，以便在数据变化时更新图表
                    ...
                    this.createTestsetElectricStatistics = function(){
                        ...
                        $('#testsetElectricReportSettingModal').modal('hide');
                    };
                    
                    this.statisticsTimeByProjectIdAndTestsetIdSuccessFunction=function(data){
                      self.testsetElectricSeriesData.removeAll();
                        if(data != null && data.status === 1){
                            ...
                            Object.entries(dataMaps).forEach(([key, value]) => {
                              ...
                              self.testsetElectricSeriesData.push(temp);
                            });
                        

                            // self.testsetElectricSumTimesData.removeAll();
                            // self.testsetElectricNamesData.removeAll();
                            // //定义一个数组
                            // executionStatus.forEach(item => { 
                            //     self.testsetElectricSumTimesData.push(item.sumTime);
                            //     self.testsetElectricNamesData.push(item.testsetName+"|(成功:"+item.sumSuccessCheckPointCount+",失败:"+item.sumFailCheckPointCount+")");
                            // });
                    ...
                    this.attached = function(view, parent) {
                        $('#testsetElectricSearchBeginDate').datepicker({
                            format: "yyyy-mm-dd",
                            todayHighlight: true,
                            language: "zh-CN",
                            autoclose: true
                        });
                        $('#testsetElectricSearchEndDate').datepicker({
                            format: "yyyy-mm-dd",
                            todayHighlight: true,
                            language: "zh-CN",
                            autoclose: true
                        });
                        $('#testsetElectricReportSettingModal').on('shown.bs.modal', function() {
                            // self.updateChart();
                        });			
                        self.initChart(); 
                    ...
                    this.activate = function() {
                      self.aotoFirst=true;
                        ...
                        self.testsetElectricAutoFlag(true);
                        self.startAutoRefresh();
                       
                    };
                }
                return new TestsetElectricReportViewModel();
            });

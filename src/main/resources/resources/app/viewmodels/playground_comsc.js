		...
		function ComPlaygroundViewModel() {
				...
				webix.ready(function () {
					self.tree = webix.ui({
						...
						rows: [
							{
								view: "text",
								id: "comscCommandTreeSearch", // 搜索框唯一ID
								placeholder: "搜索...",
								on: {
									onTimedKeyPress: function () {
										const value = this.getValue().toLowerCase();
										const tree = $$("comscCommandTree"); // 对应树的ID
										if (tree) {
											tree.filter(obj =>
												obj.value.toLowerCase().includes(value) ||
												String(obj.id).toLowerCase().includes(value)
			...
			this.checkedCmdNodeIds = function (checkedIds, checkedNodes, antbotName) {
				for (var i = 0; i < checkedIds.length; i++) {
					const commandTree = webix.$$("comscCommandTree"); // 使用 Webix API 根据ID获取树实例

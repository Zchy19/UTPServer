define(['jquery', 'durandal/plugins/http', 'komapping', 'services/executionManager', 'services/projectManager', 'services/protocolService', 'services/loginManager', 'services/selectionManager', 'services/viewManager',
	...
	function ($, $http, komapping, executionManager, projectManager, protocolService, loginManager, selectionManager, viewManager, utpService, ursService, cmdConvertService, notificationService, utilityService, JSONEditor, blockUI, bootstrapSwitch, ko, sortable, knockstrap, _,specialTestService) {
		...
		function SpecialTestThreeControlViewModel() {
			var self = this;
			...
			this.selectedSpecialTestThreeset = null;
			...
			this.startthreeSubscription = this.specialTestService.startthree.subscribe((newValue) => {
				if (newValue === true) {
					new Promise((resolve, reject) => {
						...
						self.specialTestService.startthree(false);
					}).catch((error) => {
						console.error("prepareExecution 执行出错:", error);
						self.specialTestService.startthree(false);
					});
			...
			this.clearDate = function(){
				for (let i = 0; i < self.outputSpecialTestThreeControlData().length; i++) {
					for (let j = 0; j < self.outputSpecialTestThreeControlData()[i].length; j++) {
						self.outputSpecialTestThreeControlData()[i][j].outputValue(null);
					}
				}
			};
			...
			this.prepareExecution = function () {
				...
				if (engineName == null) {
					...
					self.specialTestService.flagthree(false);
					return;
				}
			...
			this.stratExecution = function () {
				...
				var obj = {
					...
					isDummyRun: false,
					scriptIds: [self.selectedSpecialTestThreeset.scriptId],
					scriptGroupId: "0",
					testObject: "",
				...
				if (data && data.status === 1 && data.result) {
					...
					for (let j = 0; j < data.result.length; j++) {
						...
						for (let k = 0; k < executionDataResultJsonDatas.length; k++) {
							...
							// 对根据用例解析的数组 outputSpecialTestThreeControlData 进行遍历
							for (let i = 0; i < self.outputSpecialTestThreeControlData().length; i++) {
								for (let l = 0; l < self.outputSpecialTestThreeControlData()[i].length; l++) {
									var outputSpecialTestThreeControlData = self.outputSpecialTestThreeControlData()[i][l];
									if (outputSpecialTestThreeControlData.outputName === executionDataResultJsonData.varName) {
										if (executionDataResultJsonData.varValue == "NILL") {
											outputSpecialTestThreeControlData.outputValue(null);
										}
										outputSpecialTestThreeControlData.outputValue(executionDataResultJsonData.varValue);
									}
								}
							}
						}
					}
					self.specialTestThreeDataAdd();
					self.specialTestService.isExecuting(false);
					self.specialTestService.flagthree(false);
				} else {
			...
			//执行完成后添加数据
			this.specialTestThreeDataAdd = function () {
				var data = {
					specialTestId: self.selectedSpecialTestThreeset.id,
					jsonData: JSON.stringify(komapping.toJS(self.outputSpecialTestThreeControlData())),
					startTime: self.startTime
				}
				self.utpService.createSpecialTestData(data, self.specialTestThreeDataAddSuccessFunction, self.specialTestThreeDataAddErrorFunction);
			};
			this.specialTestThreeDataAddSuccessFunction = function (data) {
				if (data && data.status === 1) {
					// notificationService.showSuccess('添加执行数据成功！');
				} else {
					self.specialTestThreeDataAddErrorFunction();
				}
			}
			this.specialTestThreeDataAddErrorFunction = function () {
			...
			this.getOutputHistoryConfig = function () {
				$('#specialTestThreeExecutoryModalLabel').modal('show');
				self.utpService.getSpecialTestDatasBySpecialTestId(self.selectedSpecialTestThreeset.id, self.getOutputHistoryConfigSuccessFunction, self.getOutputHistoryConfigErrorFunction);
			}
			...
			this.outputSpecialTestThreeControlData = ko.observableArray([]);
			this.inputSpecialTestThreeControlData = ko.observableArray([]);
			//获取脚本信息
				...
				if (data && data.status === 1 && data.result) {
					...
					if (self.currentScript.script != null && self.currentScript.script != '') {
						...
						if (self.curControlCmdInfoArray == null || self.curControlCmdInfoArray.length == 0) {
							...
							self.outputSpecialTestThreeControlData.removeAll();
							self.inputSpecialTestThreeControlData.removeAll();
							let group = [];
							for (let i = 0; i < self.curControlCmdInfoArray.length; i++) {
								...
								if (self.curControlCmdInfoArray[i].monitorDataName) {
									...
									// 每四个一组，添加到outputSpecialTestThreeControlData
									if (group.length === 4) {
										self.outputSpecialTestThreeControlData.push(group);
										group = [];
									}
								}
							}

							// 如果最后一组不足三个，也添加到 outputSpecialTestThreeControlData
							if (group.length > 0) {
								self.outputSpecialTestThreeControlData.push(group);
							}
						}
			...
			this.getExecutionNodeSuccessFunction = function (response) {
				if (response && response.result && response.engineStatus) {
					...
					if (response.engineStatuses && response.engineStatuses.length > 0) {
						...
						var selectedSpecialTestThreesetName = self.selectedSpecialTestThreeset.name
						//根据"-""分割，取第一个元素"
						selectedSpecialTestThreesetName = selectedSpecialTestThreesetName.split("-")[0];
						var engineNametemp = ""
						for (let i = 0; i < engineStatuses.length; i++) {
							engineNametemp = engineStatuses[i].engineName.split("-")[0]
							if (engineNametemp === selectedSpecialTestThreesetName) {
						...
						if (self.allEngineNames().length == 3) {
							self.engineName(self.allEngineNames()[2].engineName);
						}
						// self.engineName(self.allEngineNames()[0].engineName);
					}
				} else {
			...
			this.attached = function (view, parent) {
				self.specialTestService.isExecuting(false);
				self.specialTestService.flagthree(false);
				self.selectedSpecialTestThreeset = self.viewManager.selectedSpecialTestsetActiveData;
				//获取脚本信息
				self.getScript();
				//获取所有的执行器
				self.getExecutionNode();
		...
		return new SpecialTestThreeControlViewModel();
	});

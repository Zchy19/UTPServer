define(['knockout', 'jquery', 'komapping',
	...
	function (ko, $, komapping,
		...
		function PlaygroundViewModel() {
			...
			this.commandDisplay = true; //是否显示禁用命令
						...
						rows: [
							{
								...
								on: {
									onTimedKeyPress: function () {
										...
										// 确保 self.subScriptTree 是一个 Webix 树对象
										var tree = $$("subScriptTree");
										if (tree) {
											tree.filter(function (obj) {
												var id = String(obj.id).toLowerCase();
												return obj.value.toLowerCase().indexOf(value) !== -1 || id.indexOf(value) !== -1;
											});
										}
									}
								}
							},
							{
								...
								template: function (obj, com) {
									var content = com.icon(obj, com) + (obj.$count === 0 && obj.type === "folder" ? "<div class='webix_tree_folder'></div>" : com.folder(obj, com));
			...
			this.recurPrepareCmdTreeData = function (cmdList, parentId, parentData, parentDisable) {
				for (var i = 0; i < cmdList.length; i++) {
					var currentItem = cmdList[i];
					// 跳过显式禁用的项
					if ('Disable' in currentItem && currentItem.Disable === true) {
						continue;
					}

					// 计算当前项的禁用状态
					var isDisabled = parentDisable || (currentItem.EnableLevel > self.auth);

					// 根据显示设置过滤禁用项
					if (!self.commandDisplay && isDisabled) {
						continue; // 不显示禁用项时直接跳过
					}

					var id = parentId + "-" + i.toString();

					// 初始化启用等级
					if (!('EnableLevel' in currentItem)) {
						currentItem.EnableLevel = 0;
					}

					// 分组节点处理
					if ("CommandList" in currentItem && "GroupName" in currentItem) {
						var childData = [];

						// 递归处理子节点（始终传递实际禁用状态）
						self.recurPrepareCmdTreeData(currentItem.CommandList, id, childData, isDisabled);

						// 当需要显示时，即使子节点为空也保留分组（根据需求调整）
						parentData.push({
							id: id,
							data: childData,
							value: isDisabled
								? `<span class="pull-left-container text-muted">${currentItem.GroupName}(暂无权限,不可使用)</span>`
					...
					// 叶子节点处理
						...
						var formattedCommandString = self.needRecordSetConfigAndSelectAllCmd()
							? cmdConvertService.convertRecordAllCmdToUserLanguage(self.selectedAgent.antbotType, currentItem.CmdName)
							: cmdConvertService.convertCmdToUserLanguange(
								self.selectedAgent.antbotType,
								currentItem.Type,
								currentItem.CmdName,
								currentItem.Params
							);
						...
						var commandObj = {
							commandName: currentItem.CmdName,
							commandParameters: currentItem.Params
						};
						...
						parentData.push({
							...
							cmd: commandObj,
							value: isDisabled
								? `<span class="pull-left-container text-muted">${formattedCommandString}---${currentItem.CmdName} (暂无权限,不可使用)</span>`
								: `${formattedCommandString}---${currentItem.CmdName}`,
							disabled: isDisabled
						});
					}
				}
			...
			this.attached = function (view, parent) {
		        	var commandConfig = self.systemConfig.getAuthByFeatureName('utpclient.testcommand.max_enable_level') || {};
				var hideDisabledCmd = typeof commandConfig.hideDisabledCmd === 'boolean'
					? commandConfig.hideDisabledCmd
					: false; // 无配置时默认不隐藏
				self.auth = typeof commandConfig.auth === 'number' ? commandConfig.auth : 0; // 类型检查+默认值
				self.commandDisplay = !hideDisabledCmd; // 反转布尔值				// 关闭工具箱中不符合权限的块

define(['jquery', 'durandal/plugins/http', 'komapping', 'services/executionManager', 'services/projectManager', 'services/protocolService', 'services/loginManager', 'services/selectionManager', 'services/viewManager',
	...
	function ($, $http, komapping, executionManager, projectManager, protocolService, loginManager, selectionManager, viewManager, utpService, ursService, cmdConvertService, notificationService, utilityService, JSONEditor, blockUI, bootstrapSwitch, ko, sortable, knockstrap, _) {
		...
		function SpecialTestControlViewModel() {
			var self = this;
			...
			this.specialTestClass = ko.observable('');

			this.updateSpecialTestClass = function() {
				if (self.hasNG()) {
					self.specialTestClass('bg-danger');
				}
				if (self.hasOK() && !self.hasNG()){
					self.specialTestClass('bg-success');
				}
				
				console.log(self.specialTestClass());
			};

			 // 检查是否有任何'NG'
			this.hasNG = function() {
				for (let i = 0; i < self.outputSpecialTestControlData()[0].length; i++){
					if(self.outputSpecialTestControlData()[0][i].outputValue() == 'NG'){
						return true;
					}
				}
				return false;
			...
			this.hasOK = function() {
				for (let i = 0; i < self.outputSpecialTestControlData()[0].length; i++){
					if(self.outputSpecialTestControlData()[0][i].outputValue() == 'OK'){
						return true;
					}
				}
				return false;
			...
			this.prepareExecution = function () {
				// 清除数据
				for (let i = 0; i < self.outputSpecialTestControlData().length; i++) {
					for (let j = 0; j < self.outputSpecialTestControlData()[i].length; j++) {
						self.outputSpecialTestControlData()[i][j].outputValue(null);
					}
				}
				// 设置按钮为不可点击状态
				self.isExecuting(true);
				self.specialTestClass('');
			...
			this.stratExecution = function () {
				...
				var obj = {
					...
					isDummyRun: false,
					scriptIds: [self.selectedSpecialTestset.scriptId],
					scriptGroupId: "0",
					testObject: "",
				...
				if (data && data.status === 1 && data.result) {
					...
					for (let j = 0; j < data.result.length; j++) {
						...
						for (let k = 0; k < executionDataResultJsonDatas.length; k++) {
							...
							// 对根据用例解析的数组 outputSpecialTestControlData 进行遍历
							for (let i = 0; i < self.outputSpecialTestControlData().length; i++) {
								for (let l = 0; l < self.outputSpecialTestControlData()[i].length; l++) {
									var outputSpecialTestControlData = self.outputSpecialTestControlData()[i][l];
									if (outputSpecialTestControlData.outputName === executionDataResultJsonData.varName) {
										if (executionDataResultJsonData.varValue == "NILL") {
											outputSpecialTestControlData.outputValue(null);
										}
										outputSpecialTestControlData.outputValue(executionDataResultJsonData.varValue);
									}
								}
							}
						}
					}
					self.specialTestDataAdd();
					self.isExecuting(false);
			...
			//执行完成后添加数据
			this.specialTestDataAdd = function () {
				var data = {
					specialTestId: self.selectedSpecialTestset.id,
					jsonData: JSON.stringify(komapping.toJS(self.outputSpecialTestControlData())),
					startTime: self.startTime
				}
				self.utpService.createSpecialTestData(data, self.specialTestDataAddSuccessFunction, self.specialTestDataAddErrorFunction);
			};
			this.specialTestDataAddSuccessFunction = function (data) {
				if (data && data.status === 1) {
					// notificationService.showSuccess('添加执行数据成功！');
				} else {
					self.specialTestDataAddErrorFunction();
				}
			}
			this.specialTestDataAddErrorFunction = function () {
			...
			this.getOutputHistoryConfig = function () {
				$('#specialTestExecutoryModalLabel').modal('show');
				self.utpService.getSpecialTestDatasBySpecialTestId(self.selectedSpecialTestset.id, self.getOutputHistoryConfigSuccessFunction, self.getOutputHistoryConfigErrorFunction);
			}
			...
			this.outputSpecialTestControlData = ko.observableArray([]);
			this.inputSpecialTestControlData = ko.observableArray([]);
			//获取脚本信息
				...
				if (data && data.status === 1 && data.result) {
					...
					if (self.currentScript.script != null && self.currentScript.script != '') {
						...
						if (self.curControlCmdInfoArray == null || self.curControlCmdInfoArray.length == 0) {
							...
							self.outputSpecialTestControlData.removeAll();
							self.inputSpecialTestControlData.removeAll();
							let group = [];
							for (let i = 0; i < self.curControlCmdInfoArray.length; i++) {
								...
								if (self.curControlCmdInfoArray[i].monitorDataName) {
									...
									// 每二十个一组，添加到outputSpecialTestControlData
									if (group.length === 20) {
										self.outputSpecialTestControlData.push(group);
										group = [];
									}
								}
							}
			
							// 如果最后一组不足三个，也添加到 outputSpecialTestControlData
							if (group.length > 0) {
								self.outputSpecialTestControlData.push(group);
							}
						}
			...
			this.getExecutionNodeSuccessFunction = function (response) {
				if (response && response.result && response.engineStatus) {
					...
					if (response.engineStatuses && response.engineStatuses.length > 0) {
						...
						var selectedSpecialTestsetName=self.selectedSpecialTestset.name
						//根据"-""分割，取第一个元素"
						selectedSpecialTestsetName=selectedSpecialTestsetName.split("-")[0];
						var engineNametemp =""
						for (let i = 0; i < engineStatuses.length; i++) {
							engineNametemp=engineStatuses[i].engineName.split("-")[0]
							if ( engineNametemp=== selectedSpecialTestsetName) {
		...
		return new SpecialTestControlViewModel();
	});

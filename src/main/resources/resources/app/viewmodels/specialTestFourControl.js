define(['jquery', 'durandal/plugins/http', 'komapping', 'services/executionManager', 'services/projectManager', 'services/protocolService', 'services/loginManager', 'services/selectionManager', 'services/viewManager',
	...
	function ($, $http, komapping, executionManager, projectManager, protocolService, loginManager, selectionManager, viewManager, utpService, ursService, cmdConvertService, notificationService, utilityService, JSONEditor, blockUI, bootstrapSwitch, ko, sortable, knockstrap, _,specialTestService) {
		...
		function SpecialTestFourControlViewModel() {
			var self = this;
			...
			this.selectedSpecialTestFourset = null;
			...
			this.startfourSubscription = this.specialTestService.startfour.subscribe((newValue) => {
				if (newValue === true) {
					new Promise((resolve, reject) => {
						...
						self.specialTestService.startfour(false);
					}).catch((error) => {
						console.error("prepareExecution 执行出错:", error);
						self.specialTestService.startfour(false);
					});
			...
			this.clearDate = function(){
				for (let i = 0; i < self.outputSpecialTestFourControlData().length; i++) {
					for (let j = 0; j < self.outputSpecialTestFourControlData()[i].length; j++) {
						self.outputSpecialTestFourControlData()[i][j].outputValue(null);
					}
				}
			};
			...
			this.prepareExecution = function () {
				...
				if (engineName == null) {
					...
					self.specialTestService.flagfour(false);
					return;
				}
			...
			this.stratExecution = function () {
				...
				var obj = {
					...
					isDummyRun: false,
					scriptIds: [self.selectedSpecialTestFourset.scriptId],
					scriptGroupId: "0",
					testObject: "",
				...
				if (data && data.status === 1 && data.result) {
					...
					for (let j = 0; j < data.result.length; j++) {
						...
						for (let k = 0; k < executionDataResultJsonDatas.length; k++) {
							...
							// 对根据用例解析的数组 outputSpecialTestFourControlData 进行遍历
							for (let i = 0; i < self.outputSpecialTestFourControlData().length; i++) {
								for (let l = 0; l < self.outputSpecialTestFourControlData()[i].length; l++) {
									var outputSpecialTestFourControlData = self.outputSpecialTestFourControlData()[i][l];
									if (outputSpecialTestFourControlData.outputName === executionDataResultJsonData.varName) {
										if (executionDataResultJsonData.varValue == "NILL") {
											outputSpecialTestFourControlData.outputValue(null);
										}
										outputSpecialTestFourControlData.outputValue(executionDataResultJsonData.varValue);
									}
								}
							}
						}
					}
					self.specialTestFourDataAdd();
					self.specialTestService.isExecuting(false);
					self.specialTestService.flagfour(false);
				} else {
			...
			//执行完成后添加数据
			this.specialTestFourDataAdd = function () {
				var data = {
					specialTestId: self.selectedSpecialTestFourset.id,
					jsonData: JSON.stringify(komapping.toJS(self.outputSpecialTestFourControlData())),
					startTime: self.startTime
				}
				self.utpService.createSpecialTestData(data, self.specialTestFourDataAddSuccessFunction, self.specialTestFourDataAddErrorFunction);
			};
			this.specialTestFourDataAddSuccessFunction = function (data) {
				if (data && data.status === 1) {
					// notificationService.showSuccess('添加执行数据成功！');
				} else {
					self.specialTestFourDataAddErrorFunction();
				}
			}
			this.specialTestFourDataAddErrorFunction = function () {
			...
			this.getOutputHistoryConfig = function () {
				$('#specialTestFourExecutoryModalLabel').modal('show');
				self.utpService.getSpecialTestDatasBySpecialTestId(self.selectedSpecialTestFourset.id, self.getOutputHistoryConfigSuccessFunction, self.getOutputHistoryConfigErrorFunction);
			}
			...
			this.outputSpecialTestFourControlData = ko.observableArray([]);
			this.inputSpecialTestFourControlData = ko.observableArray([]);
			//获取脚本信息
				...
				if (data && data.status === 1 && data.result) {
					...
					if (self.currentScript.script != null && self.currentScript.script != '') {
						...
						if (self.curControlCmdInfoArray == null || self.curControlCmdInfoArray.length == 0) {
							...
							self.outputSpecialTestFourControlData.removeAll();
							self.inputSpecialTestFourControlData.removeAll();
							let group = [];
							for (let i = 0; i < self.curControlCmdInfoArray.length; i++) {
								...
								if (self.curControlCmdInfoArray[i].monitorDataName) {
									...
									// 每四个一组，添加到outputSpecialTestFourControlData
									if (group.length === 4) {
										self.outputSpecialTestFourControlData.push(group);
										group = [];
									}
								}
							}

							// 如果最后一组不足三个，也添加到 outputSpecialTestFourControlData
							if (group.length > 0) {
								self.outputSpecialTestFourControlData.push(group);
							}
						}
			...
			this.getExecutionNodeSuccessFunction = function (response) {
				if (response && response.result && response.engineStatus) {
					...
					if (response.engineStatuses && response.engineStatuses.length > 0) {
						...
						var selectedSpecialTestFoursetName = self.selectedSpecialTestFourset.name
						//根据"-""分割，取第一个元素"
						selectedSpecialTestFoursetName = selectedSpecialTestFoursetName.split("-")[0];
						var engineNametemp = ""
						for (let i = 0; i < engineStatuses.length; i++) {
							engineNametemp = engineStatuses[i].engineName.split("-")[0]
							if (engineNametemp === selectedSpecialTestFoursetName) {
						...
						if (self.allEngineNames().length == 4) {
							self.engineName(self.allEngineNames()[3].engineName);
						}
						// self.engineName(self.allEngineNames()[0].engineName);
					}
				} else {
			...
			this.attached = function (view, parent) {
				self.specialTestService.isExecuting(false);
				self.specialTestService.flagfour(false);
				self.selectedSpecialTestFourset = self.viewManager.selectedSpecialTestsetActiveData;
				//获取脚本信息
				self.getScript();
				//获取所有的执行器
				self.getExecutionNode();
		...
		return new SpecialTestFourControlViewModel();
	});

define(['durandal/app', 'knockout', 'jquery', 'komapping','services/utpService', 'services/selectionManager', 
    'services/reportService','services/notificationService', 'jsoneditor', 'lodash','datepicker'],
            function(app, ko, $, komapping, utpService,selectionManager, reportService, notificationService, JSONEditor, _, datepicker) {
                ...
                function TestsetElectricReportTwoViewModel() {
                    var self = this;				
                    ...
                    this.testsetElectricTwoAutoFlag=ko.observable(false);
                    ...
                    this.getTestSetByProjectSuccessFunction = function (data) {
                      if (data != null && data.status === 1) {
                        ...
                        if (self.selectedTestset() == undefined) {
                          //默认选中第一个
                          self.selectedTestset(self.testsets()[2]);
                          if(self.aotoFirst){
                            // self.createTestsetPieChartStatistics();
                            self.createTestsetElectricTwoStatistics();
                    ...
                    this.startAutoRefresh = function() {
                        if (self.testsetElectricTwoAutoFlag()) {
                    ...
                    this.testsetElectricTwoAutoFlag.subscribe(function(newValue) {
                    ...
                    this.protocolConditionSetting = function(){
                      self.getTestSetByProject();
                        $('#testsetElectricTwoReportTwoSettingModal').modal('show');
                    };
                    this.showChart=ko.observable(false);
                    this.myChart = null;
                    this.chartOptions = {};
                    this.testsetElectricTwoSeriesData = ko.observableArray([]);
                    this.testsetElectricTwoOption = ko.computed(function() {
                      var seriesData = self.testsetElectricTwoSeriesData();
                      var totalTime = 0;
                    ...
                    this.initChart = function() {
                       var width = document.getElementById('testsetElectricTwoWidth').offsetWidth-20;
                        //设置id为testsetLine的div元素的宽高
                        document.getElementById('testsetElectricTwo').style.width = width + 'px';
                        self.myChart = echarts.init(document.getElementById('testsetElectricTwo'),'walden');
                        self.myChart.setOption(self.testsetElectricTwoOption());
                        // 订阅计算属性，以便在数据变化时更新图表
                    ...
                    this.createTestsetElectricTwoStatistics = function(){
                        ...
                        $('#testsetElectricTwoReportTwoSettingModal').modal('hide');
                    };
                    
                    this.statisticsTimeByProjectIdAndTestsetIdSuccessFunction=function(data){
                      self.testsetElectricTwoSeriesData.removeAll();
                        if(data != null && data.status === 1){
                            ...
                            Object.entries(dataMaps).forEach(([key, value]) => {
                              ...
                              self.testsetElectricTwoSeriesData.push(temp);
                            });
                        

                            // self.testsetElectricTwoSumTimesData.removeAll();
                            // self.testsetElectricTwoNamesData.removeAll();
                            // //定义一个数组
                            // executionStatus.forEach(item => { 
                            //     self.testsetElectricTwoSumTimesData.push(item.sumTime);
                            //     self.testsetElectricTwoNamesData.push(item.testsetName+"|(成功:"+item.sumSuccessCheckPointCount+",失败:"+item.sumFailCheckPointCount+")");
                            // });
                    ...
                    this.attached = function(view, parent) {
                        $('#testsetElectricTwoSearchBeginDate').datepicker({
                            format: "yyyy-mm-dd",
                            todayHighlight: true,
                            language: "zh-CN",
                            autoclose: true
                        });
                        $('#testsetElectricTwoSearchEndDate').datepicker({
                            format: "yyyy-mm-dd",
                            todayHighlight: true,
                            language: "zh-CN",
                            autoclose: true
                        });
                        $('#testsetElectricTwoReportTwoSettingModal').on('shown.bs.modal', function() {
                            // self.updateChart();
                        });			
                        self.initChart(); 
                    ...
                    this.activate = function() {
                      self.aotoFirst=true;
                        ...
                        self.testsetElectricTwoAutoFlag(true);
                        self.startAutoRefresh();
                       
                    };
                }
                return new TestsetElectricReportTwoViewModel();
            });

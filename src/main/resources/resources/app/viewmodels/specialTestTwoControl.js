define(['jquery', 'durandal/plugins/http', 'komapping', 'services/executionManager', 'services/projectManager', 'services/protocolService', 'services/loginManager', 'services/selectionManager', 'services/viewManager',
	...
	function ($, $http, komapping, executionManager, projectManager, protocolService, loginManager, selectionManager, viewManager, utpService, ursService, cmdConvertService, notificationService, utilityService, JSONEditor, blockUI, bootstrapSwitch, ko, sortable, knockstrap, _,specialTestService) {
		...
		function SpecialTestTwoControlViewModel() {
			var self = this;
			...
			this.selectedSpecialTestTwoset = null;
			...
			this.starttwoSubscription = this.specialTestService.starttwo.subscribe((newValue) => {
				if (newValue === true) {
					new Promise((resolve, reject) => {
						...
						self.specialTestService.starttwo(false);
					}).catch((error) => {
						console.error("prepareExecution 执行出错:", error);
						self.specialTestService.starttwo(false);
					});
			...
			this.clearDate = function(){
				for (let i = 0; i < self.outputSpecialTestTwoControlData().length; i++) {
					for (let j = 0; j < self.outputSpecialTestTwoControlData()[i].length; j++) {
						self.outputSpecialTestTwoControlData()[i][j].outputValue(null);
					}
				}
			};
			...
			this.prepareExecution = function () {
				...
				if (engineName == null) {
					...
					self.specialTestService.flagtwo(false);
					return;
				}
			...
			this.stratExecution = function () {
				...
				var obj = {
					...
					isDummyRun: false,
					scriptIds: [self.selectedSpecialTestTwoset.scriptId],
					scriptGroupId: "0",
					testObject: "",
				...
				if (data && data.status === 1 && data.result) {
					...
					for (let j = 0; j < data.result.length; j++) {
						...
						for (let k = 0; k < executionDataResultJsonDatas.length; k++) {
							...
							// 对根据用例解析的数组 outputSpecialTestTwoControlData 进行遍历
							for (let i = 0; i < self.outputSpecialTestTwoControlData().length; i++) {
								for (let l = 0; l < self.outputSpecialTestTwoControlData()[i].length; l++) {
									var outputSpecialTestTwoControlData = self.outputSpecialTestTwoControlData()[i][l];
									if (outputSpecialTestTwoControlData.outputName === executionDataResultJsonData.varName) {
										if (executionDataResultJsonData.varValue == "NILL") {
											outputSpecialTestTwoControlData.outputValue(null);
										}
										outputSpecialTestTwoControlData.outputValue(executionDataResultJsonData.varValue);
									}
								}
							}
						}
					}
					self.specialTestTwoDataAdd();
					self.isExecuting(false);
					self.specialTestService.flagtwo(false);
				} else {
			...
			//执行完成后添加数据
			this.specialTestTwoDataAdd = function () {
				var data = {
					specialTestId: self.selectedSpecialTestTwoset.id,
					jsonData: JSON.stringify(komapping.toJS(self.outputSpecialTestTwoControlData())),
					startTime: self.startTime
				}
				self.utpService.createSpecialTestData(data, self.specialTestTwoDataAddSuccessFunction, self.specialTestTwoDataAddErrorFunction);
			};
			this.specialTestTwoDataAddSuccessFunction = function (data) {
				if (data && data.status === 1) {
					// notificationService.showSuccess('添加执行数据成功！');
				} else {
					self.specialTestTwoDataAddErrorFunction();
				}
			}
			this.specialTestTwoDataAddErrorFunction = function () {
			...
			this.getOutputHistoryConfig = function () {
				$('#specialTestTwoExecutoryModalLabel').modal('show');
				self.utpService.getSpecialTestDatasBySpecialTestId(self.selectedSpecialTestTwoset.id, self.getOutputHistoryConfigSuccessFunction, self.getOutputHistoryConfigErrorFunction);
			}
			...
			this.outputSpecialTestTwoControlData = ko.observableArray([]);
			this.inputSpecialTestTwoControlData = ko.observableArray([]);
			//获取脚本信息
				...
				if (data && data.status === 1 && data.result) {
					...
					if (self.currentScript.script != null && self.currentScript.script != '') {
						...
						if (self.curControlCmdInfoArray == null || self.curControlCmdInfoArray.length == 0) {
							...
							self.outputSpecialTestTwoControlData.removeAll();
							self.inputSpecialTestTwoControlData.removeAll();
							let group = [];
							for (let i = 0; i < self.curControlCmdInfoArray.length; i++) {
								...
								if (self.curControlCmdInfoArray[i].monitorDataName) {
									...
									// 每四个一组，添加到outputSpecialTestTwoControlData
									if (group.length === 4) {
										self.outputSpecialTestTwoControlData.push(group);
										group = [];
									}
								}
							}

							// 如果最后一组不足三个，也添加到 outputSpecialTestTwoControlData
							if (group.length > 0) {
								self.outputSpecialTestTwoControlData.push(group);
							}
						}
			...
			this.getExecutionNodeSuccessFunction = function (response) {
				if (response && response.result && response.engineStatus) {
					...
					if (response.engineStatuses && response.engineStatuses.length > 0) {
						...
						var selectedSpecialTestTwosetName = self.selectedSpecialTestTwoset.name
						//根据"-""分割，取第一个元素"
						selectedSpecialTestTwosetName = selectedSpecialTestTwosetName.split("-")[0];
						var engineNametemp = ""
						for (let i = 0; i < engineStatuses.length; i++) {
							engineNametemp = engineStatuses[i].engineName.split("-")[0]
							if (engineNametemp === selectedSpecialTestTwosetName) {
						...
						if (self.allEngineNames().length == 2) {
							self.engineName(self.allEngineNames()[1].engineName);
						}
						// self.engineName(self.allEngineNames()[0].engineName);
					}
				} else {
			...
			this.attached = function (view, parent) {
				self.isExecuting(false);
				self.specialTestService.flagtwo(false);
				self.selectedSpecialTestTwoset = self.viewManager.selectedSpecialTestsetActiveData;
				//获取脚本信息
				self.getScript();
				//获取所有的执行器
				self.getExecutionNode();
		...
		return new SpecialTestTwoControlViewModel();
	});

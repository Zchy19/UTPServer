define(['later'], function() {
	function cronExpressionValidator() {
		...
		this.laterValidator = function(cronExpression){
			if(cronExpression){
				var cronArray = cronExpression.split(" ");
				if(cronArray.length > 7 || cronArray.length < 6)
					return false;
				
				try{
				    var cronSched = later.parse.cron(cronExpression, true);
				    if (cronSched){
				        return true;
				    }			    
				}
	    ...
	    this.cronValidate = function(cronExpression){	       
